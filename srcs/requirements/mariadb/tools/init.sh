...
if [ -d /var/lib/mysql/mysql ]; then
	...
	CREATE DATABASE IF NOT EXISTS \`$RUBY_DATABASE\` CHARACTER SET utf8 COLLATE utf8_general_ci;
	GRANT ALL ON \`$RUBY_DATABASE\`.* to '$MYSQL_USER'@'%' IDENTIFIED BY '$MYSQL_PASSWORD';
	GRANT ALL ON \`$RUBY_DATABASE\`.* to '$MYSQL_USER'@'localhost' IDENTIFIED BY '$MYSQL_PASSWORD';
EOF
